...
CEOP
     ...
     &     shiPrandtl, shiSchmidt, shiKinVisc
      ...
      Real*8 SHELFICE_dumpFreq, SHELFICE_taveFreq
      Real*8 SHELFICEheatTransCoeff
      Real*8 SHELFICEsaltTransCoeff
      Real*8 SHELFICElatentHeat
      Real*8 SHELFICEheatCapacity_Cp
      Real*8 rhoShelfice
      Real*8 SHELFICEkappa
      Real*8 SHELFICEDragLinear
      Real*8 SHELFICEDragQuadratic
      Real*8 SHELFICEthetaSurface
      Real*8 shiCdrag, shiZetaN, shiRc
      Real*8 shiPrandtl, shiSchmidt, shiKinVisc
     ...
     &     shiTransCoeffT, shiTransCoeffS
      Real*8 shelficeMass          (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeMassInit      (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeLoadAnomaly   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeForcingT      (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeForcingS      (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shiTransCoeffT        (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shiTransCoeffS        (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
     ...
     &     shelfIceMassDynTendency
      Real*8 R_shelfIce            (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeHeatFlux      (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeFreshWaterFlux(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8
...
C     drKp1, recip_drLoc
      INTEGER I,J,K,Kp1
      INTEGER bi,bj
      Real*8 tLoc(1:sNx,1:sNy)
      Real*8 sLoc(1:sNx,1:sNy)
      Real*8 pLoc(1:sNx,1:sNy)
      Real*8 uLoc(1:sNx,1:sNy)
      Real*8 vLoc(1:sNx,1:sNy)
      Real*8 u_topdr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 v_topdr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 velSq(1:sNx,1:sNy)
      Real*8 thetaFreeze, saltFreeze, recip_Cp
      Real*8 freshWaterFlux, convertFW2SaltLoc
      Real*8 a0, a1, a2, b, c0
      Real*8 eps1, eps2, eps3, eps3a, eps4, eps5, eps6, eps7, eps8
      Real*8 cFac, rFac, dFac, fwflxFac
      Real*8 aqe, bqe, cqe, discrim, recip_aqe
      Real*8 drKp1, recip_drLoc
      Real*8 recip_latentHeat
      Real*8 tmpFac

      Real*8 ustar, ustarSq
      Real*8 uStarDiag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 uLocDiag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vLocDiag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     linear dependence of freezing point on salinity
      ...
      DO bj = myByLo(myThid), myByHi(myThid)
       DO bi = myBxLo(myThid), myBxHi(myThid)
        DO J = 1-OLy,sNy+OLy
         DO I = 1-OLx,sNx+OLx
          shelfIceHeatFlux      (I,J,bi,bj) = 0.D0
          shelfIceFreshWaterFlux(I,J,bi,bj) = 0.D0
          shelficeForcingT      (I,J,bi,bj) = 0.D0
          shelficeForcingS      (I,J,bi,bj) = 0.D0
          uStarDiag             (I,J,bi,bj) = 0.D0
          uLocDiag              (I,J,bi,bj) = 0.D0
          vLocDiag              (I,J,bi,bj) = 0.D0
         ENDDO
        ENDDO
       ENDDO
      ENDDO
...
C--  overlap into lower cell
            drKp1 = drF(K)*( 1.D0 - hFacC(I,J,K,bi,bj) )

C--  lower cell may not be as thick as required
            drKp1 = MIN( drKp1, drF(Kp1) * hFacC(I,J,Kp1,bi,bj) )
            drKp1 = MAX( drKp1, 0.D0 )
            recip_drLoc = 1.D0 /
     &           ( drF(K)*hFacC(I,J,K,bi,bj) + drKp1 )

            tLoc(I,J) = ( tLoc(I,J) * drF(K)*hFacC(I,J,K,bi,bj)
     &           + theta(I,J,Kp1,bi,bj) *drKp1 )
     &           * recip_drLoc

            sLoc(I,J) = ( sLoc(I,J) * drF(K)*hFacC(I,J,K,bi,bj)
...
C--  get velocities from topdr formulation
     ...
     &      * recip_drLoc
           ...
           IF (K.lt.Nr) then
            drKp1 = drF(K)*(1.D0-hFacS(I,J,K,bi,bj))
            drKp1 = max (drKp1, 0.D0)
            recip_drLoc = 1.0 / 
     &       (drF(K)*hFacS(I,J,K,bi,bj)+drKp1)
            v_topdr(I,J,bi,bj) = 
     &       (drF(K)*hFacS(I,J,K,bi,bj)*vVel(I,J,K,bi,bj) + 
     &        drKp1*vVel(I,J,Kp1,bi,bj)) 
     &      * recip_drLoc
           ELSE
            v_topdr(I,J,bi,bj) = 0.D0
           ENDIF
...
C     calculate friction velocity and heat and salt transfer coeffs
         DO J = 1, sNy
          DO I = 1, sNx
           K = kTopC(I,J,bi,bj)
           IF ( K .NE. 0 .AND. pLoc(I,J) .GT. 0.D0 ) THEN
...
C     code of typ (pLoc-const)/pLoc
     ...
     &           *shiTransCoeffS(i,j,bi,bj) * fwflxFac
            ...
            recip_aqe = 0.D0
            IF ( aqe .NE. 0.D0 ) recip_aqe = 0.5D0/aqe
c           bqe = eps1*eps6 + eps3*eps7 - eps2
     ...
     &           + eps8*( a0*sLoc(I,J) - eps7 )
            ...
            discrim = bqe*bqe - 4.D0*aqe*cqe
            saltFreeze = (- bqe - SQRT(discrim))*recip_aqe
            IF ( saltFreeze .LT. 0.D0 )
     &           saltFreeze = (- bqe + SQRT(discrim))*recip_aqe
