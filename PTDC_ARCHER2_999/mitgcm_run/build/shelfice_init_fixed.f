...
C $Header: /u/gcmpack/MITgcm/pkg/shelfice/shelfice_init_fixed.F,v 1.16 2015/02/15 00:23:03 mlosch Exp $
...
C-----------------------------------------------------------------------
     ...
     &         R_shelfIce(i,j,bi,bj).LT.zeroRS ) THEN
          ...
          ELSE
            kTopC(i,j,bi,bj) = 0
          ENDIF
          shelficeMassInit   (i,j,bi,bj) = 0.D0
          shelficeLoadAnomaly(i,j,bi,bj) = 0.D0
          shelfIceMassDynTendency(i,j,bi,bj) = 0.D0
         ENDDO
        ENDDO
       ENDDO
      ENDDO
     ...
     &       - rhoConst*Ro_surf(i,j,bi,bj)
       ...
       ENDDO
      ENDIF
      CALL EXCH_XY_RL (  shelficeMassInit,  myThid  )
...
C    calculation when useDynP_inEos_Zc
     ...
     &      *(shelficeMassInit(i,j,bi,bj)+rhoConst*Ro_surf(i,j,bi,bj))
       ...
       ENDDO
c     ELSE
c      CALL EXCH_XY_RS (  shelficeLoadAnomaly,  myThid  )
c     ENDIF
