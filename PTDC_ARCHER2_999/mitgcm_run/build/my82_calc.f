...
C     (M. Satoh, p.315)
      ...
      Real*8    RiMax
      Real*8    MYhblScale
      Real*8    MYviscMax, MYdiffMax
      Real*8    MYdumpFreq, MYtaveFreq
      COMMON /MY_PARMS_R/     
     ...
     &     MYdumpFreq, MYtaveFreq
      ...
      Real*8 MYhbl    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,   nSx,nSy)
      Real*8 MYviscAr (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 MYdiffKr (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
C                later GM of M. Satoh, Eq. (11.3.44)
      ...
      Real*8     RiFlux, RiTmp
      Real*8     SHtmp, bTmp, tkesquare, tkel
      Real*8     RiNumber(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     GH(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     GM(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     SH(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     SM(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     tke(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
CEOP
      iMin = 2-OLx
      iMax = sNx+OLx-1
      jMin = 2-OLy
...
C     Circulation models, Springer, 2004: Eq. (11.3.60)
         RiFlux =( btmp - SQRT(btmp*btmp - 4.D0 *beta2*beta3*RiTmp) )
     &        /(2.D0*beta2)
C     M. Satoh: Eq. (11.3.58)
         SHtmp       = (alpha1-alpha2*RiFlux)/(1.D0-RiFlux)
         SH(I,J,K)   = SHtmp
...
C     compute boundary length scale MYhbl
      DO J=jMin,jMax
       DO I=iMin,iMax
        IF ( GH(I,J) .EQ. 0.D0 ) THEN
         MYhbl(I,J,bi,bj) = 0.D0
        ELSE
