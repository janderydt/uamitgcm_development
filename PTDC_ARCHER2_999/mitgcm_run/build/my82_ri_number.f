...
C     vertShear - (output) vertical shear of velocity
      ...
      Real*8     buoyFreq  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     vertShear (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C     calculate (du/dz)^2 + (dv/dz)^2 on W (between T) points.
      DO J= jMin, jMax
       DO I = iMin, iMax
        tempu= .5D0*(  uVel(I,J,Km1,bi,bj)+uVel(I+1,J,Km1,bi,bj)
     &            - (uVel(I,J,K  ,bi,bj)+uVel(I+1,J,K  ,bi,bj)) )
     &       *recip_drC(K)
        tempv= .5D0*(  vVel(I,J,Km1,bi,bj)+vVel(I,J+1,Km1,bi,bj)
